...
exports.getOrderDetails = async (req, res) => {
  try {
    ...
    const originalSubtotal = order.orderedItem.reduce(
      (sum, item) =>
        sum +
        (item.finalTotalProductPrice || item.productPrice * item.quantity),
      0
    );
    ...
    const processedItems = order.orderedItem.map((item) => {
      const itemTotal =
        item.totalProductPrice || item.productPrice * item.quantity;
      const discountProportion = originalSubtotal
        ? itemTotal / originalSubtotal
        : 0;
  ...
  try {
    ...
    if (currentStatus === "Delivered" && orderStatus !== "Returned") {
      return res
        ...
        .json({
          success: false,
          message: "Delivered orders can only be changed to Returned",
        });
    }
    ...
    if (paymentStatus && !isOnlinePayment) {
      ...
      return res
        ...
        .json({
          success: false,
          message: "Cannot change payment status for online payments",
        });
    }
  ...
  try {
    ...
    if (currentStatus === "Delivered" && productStatus !== "Returned") {
      return res
        ...
        .json({
          success: false,
          message: "Delivered items can only be changed to Returned",
        });
    }
    if (["Cancelled", "Returned"].includes(currentStatus)) {
      return res
...
exports.updatePaymentStatus = async (req, res) => {
  try {
    ...
    if (
      isOnlinePayment &&
      !["Paid", "refunded", "Partially Refunded"].includes(paymentStatus)
    ) {
      return res
        .status(400)
        .json({
          success: false,
          message:
            "Online payments can only be set to Paid, refunded, or partially-refunded",
        });
    }
    if (
      ...
      return res
        ...
        .json({
          success: false,
          message: "Cannot change Paid status to pending or failed",
        });
    }
    if (currentPaymentStatus === "failed" && paymentStatus !== "failed") {
      return res
        .status(400)
        .json({ success: false, message: "Cannot change status from failed" });
    }
    if (
      ["refunded", "Partially Refunded"].includes(currentPaymentStatus) &&
      !["refunded", "partially-refunded"].includes(paymentStatus)
    ) {
      return res
        .status(400)
        .json({
          success: false,
          message: "Cannot change status from refunded or partially-refunded",
        });
    }
  ...
  try {
    ...
    if (!item || item.productStatus !== "Return Requested") {
      return res
        ...
        .json({
          success: false,
          message: "Invalid item or no return requested",
        });
    }
    ...
    if (status === "Return Approved") {
      ...
      const orderTotalBeforeDiscount = order.orderedItem.reduce(
        (acc, i) => acc + i.finalTotalProductPrice,
        0
      );

      const orderCouponDiscount = order.couponDiscount || 0;

      const proportionalCouponShare =
        (item.finalTotalProductPrice / orderTotalBeforeDiscount) *
        orderCouponDiscount;

      const refundAmount =
        item.finalTotalProductPrice - proportionalCouponShare;
      ...
      wallet.transaction.push({
        ...
        date: new Date(),
        orderId: order._id,
        description: `Refund for return of ${
      ...
      order.orderAmount -= item.finalTotalProductPrice;
      order.couponDiscount -= proportionalCouponShare;
      order.finalAmount -= refundAmount;

      const allItemsReturned = order.orderedItem.every(
        (i) =>
          i.productStatus === "Returned" ||
          i.productStatus === "Return Approved"
      );
      if (allItemsReturned) {
        order.orderStatus = "Returned";
      }

      const allItemsRefunded = order.orderedItem.every(
        (i) => i.refunded || i.productStatus === "Cancelled"
      );
      if (allItemsRefunded) {
