...
exports.getHomePage = async (req, res) => {
  try {
    ...
    let userWishlist = [];
    console.log("getHomePage:", req.session);
    // If user is authenticated, fetch their wishlist
    if (req.session && req.session.user?.isAuth && req.session.user) {
      const user = await User.findById(req.session.user.id).lean();
      // userWishlist = user.wishlist || [];
      userWishList = req.session.user.wishlist[0];
      userWishlist =userWishList?.wishlistItems?.map((item) =>item.productId.toString())
    }
